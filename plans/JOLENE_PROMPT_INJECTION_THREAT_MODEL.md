...
| Private tools | Five registered private tools are read-only and schema-bounded; external messaging has no model tool. | Tool purpose, arguments, call chain, and per-turn budget are not intent-bound. |
