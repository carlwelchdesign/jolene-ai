...
| JOL-ARCH-010 | Partial | An explicit local registry, on-demand read-only snapshots, canonical-owner conversational tools, and an explicitly enabled local worker report project existence, plan freshness, Git branch/revision/dirty state, and clear alerts without exposing root paths, plan contents, or diffs. Scheduled checks enforce cadence, daily budget, terminal run count, pause state, and bounded visible history. An opt-in durable outbox can notify only the configured Slack owner DM on alert-set transitions, with bounded retry and no-change/manual suppression. Build verification, authenticated remote administration, production hosting, and live real-transition owner-DM evidence remain pending. |
...
| JOL-ARCH-014 | Implemented for local pilot | The ARM64 image builds successfully. Private API and Slack run as separate Compose processes from the same non-root image with a read-only application filesystem, read-only vault mount, durable SQLite volume, and no baked-in secrets. The operational cutover is complete and both processes are running; production deployment remains separate. |
