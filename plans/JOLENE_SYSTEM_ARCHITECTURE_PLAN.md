...
| JOL-ARCH-009 | Partial; research pilot, review control, and non-activating render contract complete | Initial runtime behavior prompt and non-impersonation rules exist. The rights-conscious pilot now registers 11 primary sources, codes 25 paraphrase-only observations across five sources, and independently reconciles seven observations while separating observed, inferred, and designed evidence. Carl can inspect the exact research packet and record a durable owner-only relevance decision bound to all five artifact hashes; changed artifacts make prior decisions stale. A versioned structured response contract now fingerprints grounded summary, source-linked claims, citations, limitations, next actions, completion, and permission state before a deterministic neutral or Jolene presentation pass. Eleven paired context fixtures require invariant semantic segments, no more than one flourish, and hard-fail on impersonation, borrowed quote/biography framing, dialect/default intimacy, hidden state, or sensitive-context ornament. This contract is deliberately not connected to the model, API, Slack, public delegate, or voice, so runtime behavior is unchanged. Carl's actual relevance/tuning approval, complete corpus, final behavior specification, rights review, broader evaluation suite, representative text pilot, and explicit activation remain pending. |
