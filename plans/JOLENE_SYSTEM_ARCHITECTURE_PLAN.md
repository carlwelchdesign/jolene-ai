...
| JOL-ARCH-009 | Partial; research pilot, owner decision controls, and non-activating render contract complete | Initial runtime behavior prompt and non-impersonation rules exist. The rights-conscious pilot registers 11 primary sources, codes 25 paraphrase-only observations across five sources, and independently reconciles seven observations while separating observed, inferred, and designed evidence. Carl can inspect the exact packet and record a durable owner-only relevance decision bound to all five artifact hashes. Once that exact snapshot is approved, a separate local control can record nine explicit tuning choices bound to both the research snapshot and a versioned tuning-contract hash; changed or malformed state fails closed, and neither decision activates behavior. A versioned structured response contract fingerprints grounded summary, source-linked claims, citations, limitations, next actions, completion, and permission state before deterministic neutral or Jolene presentation. Eleven paired context fixtures require invariant semantic segments, no more than one flourish, and hard-fail on impersonation, borrowed quote/biography framing, dialect/default intimacy, hidden state, or sensitive-context ornament. These controls are deliberately not connected to the model, API response path, Slack, public delegate, or voice. Carl's actual relevance and tuning decisions, complete corpus, final behavior specification, rights review, broader evaluation suite, representative text pilot, and explicit activation remain pending. |
