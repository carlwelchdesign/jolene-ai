...
| JOL-ARCH-015 | Partial; private ingestion, scaled review control, retrieval, durable reviewed-conflict registry, visual conflict review, offline export, isolated loopback delegate, explicit unresolved-conflict policy, and offline response/lifecycle/red-team/contact/conflict evaluation baseline implemented | The private SQLite registry models sources, claims, maturity, visibility, review freshness, relationships, revocation, supersession, missing sources, and owner-reviewed unresolved/resolved claim conflicts. Portfolio and bounded Obsidian imports currently contain 143 active review-required claims and zero public-approved claims. An owner-scoped loopback review screen exposes source-first decisions, registry-wide search and pagination, bounded review/fingerprint context, plus explicit two-to-five-claim conflict declaration and resolution without publishing, messaging, or semantic inference. Private retrieval provides stable chunks, optional embeddings, lexical/vector fusion, exact citations, deterministic fallback, and content-minimizing access logs. A deny-by-default offline artifact exports only eligible public-approved records with a versioned manifest, stable hash, revocations, explicit canonical unresolved-conflict groups, and leak checks; partial-public conflicts suppress eligible members without revealing private membership. The canonical artifact is valid and empty. A physically separate loopback-only reference process validates that artifact and serves manifest, answer, job-fit, and contact-staging contracts without loading private runtime dependencies. Optional grounded OpenAI answer prose is disabled by default, receives only preselected public evidence, and fails back to deterministic output. A versioned 41-case offline harness measures 24 backend invariants at precommitted blocker thresholds with non-content reports, including real store/export lifecycle, red-team egress, contact-staging, and semantic-conflict behavior. CORS, hosting, portfolio integration, full evaluation, and deployment remain absent. Authenticated remote administration, pgvector, MCP, and graph infrastructure remain evaluation-gated. |
